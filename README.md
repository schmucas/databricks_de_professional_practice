...
flowchart LR
    ...
    subgraph gh["📦 GitHub"]
        direction TB
        PR(["Pull request"])
        CHECK{{"PR checks<br/>ruff · pytest · bundle validate"}}
        MAIN(["main branch"])
...
Nothing reaches production without passing the PR checks (ruff, unit tests, and `bundle validate`) and the prod approval gate. Integration tests against staging are still planned (see the status table above).
