...
The platform runs on a fictional dataset generated by the setup notebook: **Shadowfax Logistics**, an Edoras-based Middle-earth freight company ("delivers on time, every time, even under siege"). The fleet is 200+ pony carts, freight wains and cold wagons working the routes between the Shire and Gondor, moving ~50K shipments a month for B2B and B2C customers, from the forges of Erebor to roughly half the Baggins family. Five sources feed the pipeline. Each one stands in for a different real-world system and delivery pattern, and each carries a deliberate wrinkle that exercises a specific engineering technique:

| Dataset | Mimics (real-world source) | Delivery & format | Nature & traits |
|---|---|---|---|
| **`customers`** | Operational OLTP database: a Postgres customer master (current-state, no history) | Delta table with **Change Data Feed** | ~200 customers, every one a named Middle-earth character. CDF emits insert / update / delete **post-images**; feeds the **SCD Type 2** dimension via Auto CDC. Source **deletes propagate downstream** (a GDPR-style right to erasure; some customers vanish Bilbo-style). |
...
| **`vehicle_telemetry`** | **IoT / telematics** fleet under at-least-once delivery | JSON files on a UC Volume, **Auto Loader** | ~200K sensor readings, ~every 30s per vehicle. Dedup key is `reading_id` (timestamps are not unique); volume deliberately **skewed**: 60% of rows from 5 vehicles. (Why wagons stream telemetry is a question for the wizard who did the retrofit.) |
| **`vehicles`** | Slow-moving **master data**: the stable register | JSON files on a UC Volume, **Auto Loader**; seed plus rare drops | 200 vehicles (pony carts, freight wains, cold wagons). Rare depot reassignments as full-row post-images: a plain **SCD Type 1** source. No backfill, no history, no deletes. |
...
One does not simply walk into prod: nothing deploys without passing the PR checks and the prod approval gate. Automated unit and integration tests are in progress/planned (see the status table below).
...
flowchart LR
    ...
    DDATE["<b>dim_date</b> · conformed<br/>PK date_key<br/>full_date · year · quarter · month<br/>month_name · iso_week<br/>day_of_week · day_name<br/>is_weekend · is_holiday"]
    DLOC["<b>dim_location</b><br/>PK location_sk<br/>city · realm · region"]
...
> Interactive version (with full column types): [Shadowfax Logistics Gold Star Schema on dbdiagram.io](https://dbdiagram.io/d/Shadowfax-Logistics-Gold-Star-Schema-6a4aab974ac62e474c32f352).
...
![Classic PySpark Silver to Gold pipeline run in Databricks Jobs](docs/images/classic_pyspark_silver_gold_run.png)
...
### It runs: ingestion pipeline

Here is an actual run of the ingestion path on **Databricks Free Edition (serverless)**: the incremental generator advances the `sl_ingest` sources, then Bronze picks up the fresh batch via Auto Loader with Change Data Feed enabled, so every downstream track reads the same freshly landed, deliberately messy input. This run was launched with the optional `DRIFT_THIS_RUN = true` job parameter, so it also exercises the deterministic schema-drift path (one new column added and then persisted for subsequent runs).

![Ingestion pipeline run in Databricks Jobs](docs/images/ingestion_pipeline_run.png)

---

## Environment separation

Environments are separated **at the catalog level**, within a single workspace (Free Edition provides only one). Each environment gets its own catalog, prefixed **`sl_`** (for **S**hadowfax **L**ogistics): `sl_dev`, `sl_stage`, and `sl_prod`, plus a shared `sl_ingest` source catalog. In a paid setup I'd push this to separate workspaces per environment (with catalog binding as needed), but that's beyond Free Edition.

```mermaid
flowchart TB
    subgraph UC["Unity Catalog · sl_ = Shadowfax Logistics"]
