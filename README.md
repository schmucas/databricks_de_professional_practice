...
| **`vehicle_telemetry`** | JSON in UC Volumes | ~200K **IoT / telematics** sensor readings, emitted roughly every 30s per active vehicle: speed, fuel, engine and cargo temperature, odometer. Each reading carries a `reading_id` (the telematics-gateway message id) as its dedup key, since sensor timestamps are not unique under at-least-once delivery. Volume is deliberately **skewed**: 60% of rows come from just 5 vehicles. |
