...
| **`orders_cdc`** | JSON in UC Volumes | A daily change feed — 200 new and 500 updated orders — for **MERGE / `APPLY CHANGES INTO`** upsert handling. |
...
flowchart LR
    ...
    DLTJOB["🔷 DLT pipeline refresh<br/>Silver → Gold"]
...
flowchart LR
    ...
    subgraph dbx["☁️ Databricks Workspaces"]
        direction TB
...
flowchart LR
    ...
    subgraph dlt_track["🔷 TRACK A — DELTA LIVE TABLES · declarative pipeline"]
        direction LR
        SV1["🥈 Silver · DLT<br/>APPLY CHANGES INTO<br/>Quality expectations"]
...
Each environment is fully isolated — a separate Unity Catalog, separate data, separate deployments. Catalogs are prefixed **`sl_`** (for **S**wiss**L**ogistics): `sl_dev`, `sl_stage`, and `sl_prod`, plus a shared `sl_ingest` source catalog. There is no shared mutable state between dev, stage, and prod.
...
| **Delta Live Tables** | Python `@dlt.table` — quality expectations, Auto CDC, dependency graph |
...
| **Environment separation** | Three fully isolated targets (dev / stage / prod) with separate catalogs |
...
**Notebooks for ingestion, DLT for transformation.** Bronze ingestion runs as notebook tasks in a job — simple, independently schedulable, easy to debug. DLT takes over at Silver where its Auto CDC (`APPLY CHANGES INTO`), quality expectations, and lineage tracking add the most value.
...
| **Pipelines** | Delta Live Tables (Python) + traditional PySpark |
