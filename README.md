...
| **`orders`** | An external system dropping change events as files: a classic **file-based CDC export** (a vendor extract, or a Debezium / Fivetran-style feed landing to object storage) | JSON files on a UC Volume, read with **Auto Loader** | 50K parcel and freight orders (status, payment, amount, origin / destination). Initial **snapshot** (backfill) of every order as full rows, then incremental drops: **INSERTs and UPDATEs both carry full row post-images** (an UPDATE restates the order's whole row with the new status, the way database-sourced CDC actually emits changes). ~2% **missing amounts**. Versioned for **SCD Type 2** (order-status history); no deletes. |
...
| **`vehicles`** | Slow-moving **master data**: the fleet register behind the telemetry and shipment vehicle ids | JSON files on a UC Volume, **Auto Loader**; one-time seed plus rare update drops | 200 vehicles (type, capacity, cold-chain capability, home depot). Rare depot reassignments arrive as **full-row post-images**: a plain **SCD Type 1** source feeding the vehicle dimension. No snapshot backfill, no SCD2 history, no deletes. |

**SCD Type 2, two ways.** Both `customers` and `orders` land as Type 2 history, but through deliberately different change-delivery mechanisms. `customers` arrives as a native Delta Change Data Feed read straight off an in-place-mutated table. `orders` arrives as a file-based CDC export: JSON change files (an initial snapshot plus incremental `_change_type` drops) landed on a Volume and read with Auto Loader. Both hand over complete row post-images, so both feed the same Auto CDC / SCD2 apply, keyed on the business id and sequenced by `_change_ts`. Same SCD2 logic, reached through two different landing mechanisms (native change feed vs file-based CDC), and that contrast is the point.

Together they map to the exam-relevant skills: SCD Type 2 from two different CDC sources, full-load-plus-incremental ingestion, file-based vs native-CDF change capture, downstream delete propagation for privacy compliance, data-quality enforcement, deduplication, and data-skew optimisation, both developed via the classic pyspark route and the new declarative approach.
